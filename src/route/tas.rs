...
#[cfg(test)]
mod tests {
    /// The GUI's checkpoint picker is generated by walking the route's
    /// `SeqCheckpoint` nodes — pin that the walk finds them, in route order.
    #[test]
    fn checkpoint_list_is_generated_from_the_route() {
        let checkpoints = super::create_tas().checkpoints();
        assert!(!checkpoints.is_empty());
        let before = checkpoints
            .iter()
            .position(|c| c == "Outside Forbidden Cavern")
            .expect("known checkpoint present");
        let after = checkpoints
            .iter()
            .position(|c| c == "After Bosslug")
            .expect("known checkpoint present");
        assert!(before < after, "checkpoints should be in route order");
    }
}
