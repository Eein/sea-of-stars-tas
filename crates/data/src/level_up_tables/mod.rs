...
pub fn get_sum_of_character_stats_by_level(
    ...
    match character {
        PlayerPartyCharacter::Valere => self::valere::stats_table_summed()
            .get(&level)
            .unwrap()
            .clone(),
        PlayerPartyCharacter::Zale => self::zale::stats_table_summed()
            .get(&level)
            .unwrap()
            .clone(),
        PlayerPartyCharacter::Bst => self::bst::stats_table_summed().get(&level).unwrap().clone(),
        PlayerPartyCharacter::Garl => self::garl::stats_table_summed()
            .get(&level)
            .unwrap()
            .clone(),
        PlayerPartyCharacter::Serai => self::serai::stats_table_summed()
            .get(&level)
            .unwrap()
            .clone(),
        PlayerPartyCharacter::Reshan => self::reshan::stats_table_summed()
            .get(&level)
            .unwrap()
            .clone(),
