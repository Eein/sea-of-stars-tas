...
impl MachineType {
    ...
    /// process.
    pub fn read(process: &Process, module_address: u64) -> Option<Self> {
...
pub fn read_size_of_image(process: &Process, module_address: u64) -> Option<u32> {
...
fn read_coff_header(process: &Process, module_address: u64) -> Option<(COFFHeader, u64)> {
    ...
    let coff_header_address = module_address + dos_header.e_lfanew as u64;
...
pub fn symbols(
    ...
    let dos_header = process.read::<DOSHeader>(module_address);

    let export_directory = match dos_header {
        Ok(header) => process
            .read::<u32>(address + header.e_lfanew as u64 + 0x88)
            .ok(),
        _ => None,
    };
    ...
    let symbols_def = match dos_header {
        Ok(_) => match export_directory {
            ...
            Some(export_dir) => process
                .read::<ExportedSymbolsTableDef>(address + export_dir as u64)
                .ok(),
            _ => None,
        },
        _ => None,
